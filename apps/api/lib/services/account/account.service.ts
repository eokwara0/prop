...
export class AccountService {
  ...
  async findByProvider(provider: string, providerAccountId: string) : Promise<IAccount | null> {
    const account = await this.accountModel.query().where("provider" , provider).andWhere('providerAccountId' , providerAccountId).first();
    return account.toJSON() as IAccount || null;
  }

  /** Find all accounts for a user */
  async findByUserId(userId: string) : Promise<IAccount | null > {
    return (await this.accountModel.query().where('userId', userId).first()).toJSON() as IAccount;
  }
  ...
  async updateTokens(
    ...
    tokens: Partial<Pick<IAccount, 'refresh_token' | 'access_token' | 'expires_at' | 'token_type' | 'scope' | 'id_token' | 'session_state'>>
  ) : Promise<IAccount | null>{
    const updated = await this.accountModel.query()
      .patch(tokens)
  ...
  /** Delete an account */
  async deleteAccount(provider: string, providerAccountId: string) : Promise<IAccount | null> {
    const deleted = await this.accountModel.query()
      .delete()
      .where({ provider, providerAccountId })
      .returning('*');
