...
export default function PropertyStats() {
  ...
  return (
    <div className="max-md:grid  min-md:flex  max-md:grid-cols-1 gap-4 w-full p-2">
      {  data !== undefined ?  [...Object.entries(data!).slice(2).map((a,b) => {
        return (
          <div
            key={b}
          ...
          >
            {
                b == 0 ? <PropertyStatsCard title='Property count' value={`${a[1]}`}>
                    <BirdIcon/>
                </PropertyStatsCard>  : 
                 b === 1 ? <PropertyStatsCard title='Portfolio value' value={`R${formatter.format(Number(a[1]))}`}>
                    <BadgeEuro/>
                 </PropertyStatsCard> : 
                 b === 2 ? <PropertyStatsCard title='Available Properties' value={`${a[1]}`}>
                    <CherryIcon/>
                 </PropertyStatsCard> : 
                 b === 3 ? <PropertyStatsCard title='Rented properties' value={`${a[1]}`}>
                    <DramaIcon/>
                 </PropertyStatsCard>  : <></>
            }
...
function PropertyStatsCard({title , value, children } : {title : string , value : string , children : React.ReactNode }) {
    return <div className='flex flex-col justify-between gap-10'>
